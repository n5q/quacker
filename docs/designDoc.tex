...
The application consists of the following key components:
	...
	% \subsection*{Key Data Structures and Relationships}
	% \begin{itemize}
	%     \item \textbf{User Data:} Each user is represented as an entry in the \texttt{users} table with fields such as $usr$, $name$, $email$, and $password$.
	%     \item \textbf{Quack Data:} Quacks are stored in the \texttt{tweets} table, which includes $tid$, $text$, $date$, and $time$.
	%     \item \textbf{Follows Relationship:} The \texttt{follows} table maintains follower-followee relationships.
	%     \item \textbf{Hashtag Mentions:} The \texttt{hashtag\_mentions} table tracks associations between Quacks and hashtags.
	% \end{itemize}
